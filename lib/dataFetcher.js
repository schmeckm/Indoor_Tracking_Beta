...
dotenv.config({ path: path.resolve(__dirname, "../process.env") }); // Load .env file for MQTT configurations
...
// Function to extract and restructure beacon data
...
async function sendToMongoDB(extractedData) {
    for (const item of extractedData) {
        ...
        try {
            await axios.post(`${process.env.BACKEND_URL}/temperature/addTemp`, dataToMongoDB);
...
async function fetchDataAndSend(io) {
    ...
    try {
        const response = await axios.get(`${process.env.BACKEND_URL}/position/getPosition`);
